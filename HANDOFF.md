...
- **Super-Block mid-fight interrupt:** when the meter fills *during a FIGHT* and grants a superpower, the fight PAUSES (`phase='SUPERPOWER_INTERRUPT'`) after the current lane finishes. The owning side may play *only that superpower* immediately (SP-only window) or skip; skipping keeps it in `readySuperpower` for their normal play phase. Mechanics: `combat.resolveFight` is resumable via `state.fightResume={nextLane}`; `applyHeroDamage` pushes the charging side onto `state.interrupts` (a queue — both sides can charge in one fight, offered one at a time in lane order); `reduce.finishInterruptStep` pops the queue and resumes the fight (resets `phase='FIGHT'` so `endTurn` proceeds to the next turn once the fight completes). `playSuperpower`/`advancePhase` accept the interrupt phase for the queue-front side only. Diverges from real PvZH (which just adds the SP to hand) — Ben's design.
