...
- **Net-mode combat animation is still a KNOWN DEFERRED GAP** — remote state is adopted directly (bypasses `useCombatAnimation`), so the board snaps to the final position over WiFi; only the local god-view animates. Future fix = replay `state.combatEvents` on the adopting device.
