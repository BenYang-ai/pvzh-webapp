...
describe('FIGHT — basic combat + zombie-first (§6)', () => {
  it('zombie attacks first; kills plant before it strikes back', () => {
    const s = baseState({ phase: 'FIGHT' });
    placeFighter(s, 0, 'zombie', 'z_basic'); // 3/2
    placeFighter(s, 0, 'plant', 'p_cabbagepult'); // 3/2 — would trade, but zombie first
    resolveFight(s, cfg);
    expect(s.lanes[0].plant).toBeNull(); // plant dead
    expect(s.lanes[0].zombie?.health).toBe(2); // zombie untouched (tie → zombie wins)
  });

  it('unblocked fighter hits enemy hero', () => {
    const s = baseState({ phase: 'FIGHT' });
    placeFighter(s, 1, 'zombie', 'z_basic'); // 3 atk
    resolveFight(s, cfg);
    expect(s.plant.hero.hp).toBe(17);
  });

  it('plant unblocked hits zombie hero', () => {
    const s = baseState({ phase: 'FIGHT' });
    placeFighter(s, 2, 'plant', 'p_bonkchoy'); // 4 atk
    resolveFight(s, cfg);
...
describe('FIGHT — deadly (§6, ans #5)', () => {
  it('deadly destroys a high-hp fighter on any >0 damage', () => {
    const s = baseState({ phase: 'FIGHT' });
    placeFighter(s, 0, 'zombie', 'z_toxicimp'); // 2/2 deadly
    placeFighter(s, 0, 'plant', 'p_snapdragon'); // 4/5
    resolveFight(s, cfg);
    expect(s.lanes[0].plant).toBeNull(); // 5hp snap destroyed by 2 deadly dmg
    expect(s.lanes[0].zombie?.health).toBe(2); // snap died step1, never struck back
  });
...
describe('FIGHT — frenzy (§6 STEP4, zombie only)', () => {
  it('kills blocker then bonus-attacks the hero', () => {
    const s = baseState({ phase: 'FIGHT' });
    placeFighter(s, 0, 'zombie', 'z_ninja'); // 4/2 frenzy
    placeFighter(s, 0, 'plant', 'p_peashooter'); // 2/2 → dies to 4
    resolveFight(s, cfg);
    expect(s.lanes[0].plant).toBeNull();
    expect(s.plant.hero.hp).toBe(16); // bonus 4 to hero
    expect(s.lanes[0].zombie?.health).toBe(2); // ninja survived
  });

  it('no bonus attack when blocker survives', () => {
    const s = baseState({ phase: 'FIGHT' });
    placeFighter(s, 0, 'zombie', 'z_ninja'); // 4 atk
    placeFighter(s, 0, 'plant', 'p_wallnut'); // 0/6 armored:1 → 6-(4-1)=3, survives
    resolveFight(s, cfg);
    expect(s.lanes[0].plant?.health).toBe(3);
    expect(s.plant.hero.hp).toBe(20); // no frenzy face damage
  });
});
